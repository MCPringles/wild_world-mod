...
import net.mcreator.wild_world.procedures.RichSoilOnBlockRightClickedProcedure;
...
public class RichSoilBlock extends WildWorldElements.ModElement {
	...
	public static class CustomBlock extends Block {
		public CustomBlock() {
			super(Block.Properties.create(Material.EARTH).sound(SoundType.GROUND).hardnessAndResistance(1f, 3f).lightValue(0).harvestLevel(-1)
					.harvestTool(ToolType.SHOVEL).tickRandomly());
			setRegistryName("richsoil");
		}
		...
		public boolean onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity entity, Hand hand, BlockRayTraceResult hit) {
			...
			Direction direction = hit.getFace();
			{
				...
				$_dependencies.put("z", z);
				$_dependencies.put("world", world);
				RichSoilOnBlockRightClickedProcedure.executeProcedure($_dependencies);
			}
			return true;
		}
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("richsoil", "richsoil", blockAt -> {
