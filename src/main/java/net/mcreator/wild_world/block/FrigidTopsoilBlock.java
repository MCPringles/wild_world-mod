...
import net.mcreator.wild_world.procedures.FrigidTopsoilUpdateTickProcedure;
...
public class FrigidTopsoilBlock extends WildWorldElements.ModElement {
	...
	public static class CustomBlock extends Block {
		public CustomBlock() {
			super(Block.Properties.create(Material.ORGANIC).sound(SoundType.GROUND).hardnessAndResistance(3f, 3.5f).lightValue(0).harvestLevel(0)
					.harvestTool(ToolType.SHOVEL));
			setRegistryName("frigidtopsoil");
		}
		...
		public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
			...
			return Collections.singletonList(new ItemStack(FrigidDirtBlock.block, (int) (1)));
		}
		...
		public void tick(BlockState state, World world, BlockPos pos, Random random) {
			...
			int z = pos.getZ();
			{
				...
				FrigidTopsoilUpdateTickProcedure.executeProcedure($_dependencies);
			}
			world.getPendingBlockTicks().scheduleTick(new BlockPos(x, y, z), this, this.tickRate(world));
		}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("frigidtopsoil", "frigidtopsoil", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.GRASS_BLOCK.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 32), Placement.COUNT_RANGE, new CountRangeConfig(32, 40, 40, 240)));
		}
	}
}
