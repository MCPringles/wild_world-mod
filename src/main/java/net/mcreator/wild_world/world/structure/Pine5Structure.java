...
public class Pine5Structure extends WildWorldElements.ModElement {
	public Pine5Structure(WildWorldElements instance) {
		super(instance, 178);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 1000000) {
					...
					for (int a = 0; a < count; a++) {
						...
						if (blockAt.getBlock() == Blocks.GRASS_BLOCK.getDefaultState().getBlock())
							blockCriteria = true;
						if (blockAt.getBlock() == BarrenTopsoilBlock.block.getDefaultState().getBlock())
							blockCriteria = true;
						if (blockAt.getBlock() == BarrenDirtBlock.block.getDefaultState().getBlock())
							blockCriteria = true;
						if (!blockCriteria)
							continue;
