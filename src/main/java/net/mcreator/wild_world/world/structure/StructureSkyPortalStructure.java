...
public class StructureSkyPortalStructure extends WildWorldElements.ModElement {
	public StructureSkyPortalStructure(WildWorldElements instance) {
		super(instance, 269);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 100) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("wild_world", "skyportal"));
						if (template == null)
							return false;
