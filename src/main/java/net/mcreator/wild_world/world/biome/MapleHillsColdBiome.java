...
public class MapleHillsColdBiome extends WildWorldElements.ModElement {
	@ObjectHolder("wild_world:maplehillscold")
	public static final CustomBiome biome = null;
	public MapleHillsColdBiome(WildWorldElements instance) {
		super(instance, 216);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		BiomeDictionary.addTypes(biome, BiomeDictionary.Type.FOREST, BiomeDictionary.Type.HILLS, BiomeDictionary.Type.SNOWY,
				BiomeDictionary.Type.COLD, BiomeDictionary.Type.DENSE);
		BiomeManager.addSpawnBiome(biome);
		BiomeManager.addBiome(BiomeManager.BiomeType.ICY, new BiomeManager.BiomeEntry(biome, 3));
	}
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.4f).scale(0.1f).temperature(-0.1f).precipitation(Biome.RainType.SNOW)
					.category(Biome.Category.NONE).waterColor(-16761361).waterFogColor(-16761361)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(FrigidTopsoilBlock.block.getDefaultState(),
							FrigidDirtBlock.block.getDefaultState(), FrigidDirtBlock.block.getDefaultState())));
			setRegistryName("maplehillscold");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Biome.createDecoratedFeature(new CustomTreeFeature(),
					IFeatureConfig.NO_FEATURE_CONFIG, Placement.COUNT_EXTRA_HEIGHTMAP, new AtSurfaceWithExtraConfig(7, 0.1F, 1)));
		}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == FrigidTopsoilBlock.block.getDefaultState().getBlock()
							|| ground == FrigidDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == FrigidTopsoilBlock.block.getDefaultState().getBlock()
									|| ground2 == FrigidDirtBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), FrigidDirtBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == MapleLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, MapleLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, MapleLogWildBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MapleLogWildBlock.block.getDefaultState().getBlock()
					|| blockType == MapleLeavesBlock.block.getDefaultState().getBlock()
					|| blockType == FrigidTopsoilBlock.block.getDefaultState().getBlock()
					|| blockType == FrigidDirtBlock.block.getDefaultState().getBlock();
		}
