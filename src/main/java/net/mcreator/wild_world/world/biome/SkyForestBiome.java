...
public class SkyForestBiome extends WildWorldElements.ModElement {
	...
	public class CustomBiome extends Biome {
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Biome.createDecoratedFeature(new BlossieTreeFeature(),
	...
	public class BlossieTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == BlossieLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, BlossieLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, BlossieLogWildBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlossieLogWildBlock.block.getDefaultState().getBlock()
					|| blockType == BlossieLeavesBlock.block.getDefaultState().getBlock()
